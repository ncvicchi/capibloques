...
export default function InterpreterBoard({ account, store, program, scene, board, centralDisplay, onChangeBoard, onChangeCentralDisplay, onClose }: { account: Account; store: AccountDraftStore; program: CompiledProgram; scene: SceneDefinition; board: BoardProfileId; centralDisplay?: CentralDisplayTarget; onChangeBoard: (board: BoardProfileId, scene: SceneDefinition) => void; onChangeCentralDisplay: (centralDisplay?: CentralDisplayTarget) => void; onClose: () => void }) {
  ...
  return <Dialog open onOpenChange={open => { if (!open) close(); }}><DialogContent className="firmware-dialog usb-dialog" showCloseButton={state.stage !== 'sending'}>
    ...
    {step === 'review' && <section className="board-guide-screen"><h3>Placas del proyecto</h3><div className="board-role-grid"><article className="board-role-card"><span>Ejecuta el programa</span><h4>Placa del proyecto</h4><BoardPhoto id={board} /><strong>{boardProfile(board).name}</strong><Button variant="outline" onClick={() => setStep('board')}>Cambiar placa</Button></article><article className="board-role-card"><span>Muestra y controla la escena</span><h4>Pantalla central</h4>{centralDisplay ? <><BoardPhoto id={WAVESHARE} /><strong>{boardProfile(WAVESHARE).name}</strong><p>{centralDisplay.ssid ? `Emparejada como ${centralDisplay.ssid}` : 'Todavía falta emparejar las dos placas.'}</p><Button variant="outline" onClick={() => { setPairCredentials(null); setStep('pair-screen'); }}>{centralDisplay.ssid ? 'Volver a emparejar' : 'Preparar pareja'}</Button><Button variant="outline" onClick={() => onChangeCentralDisplay(undefined)}>Quitar pantalla</Button></> : <><div className="board-guide-hero" aria-hidden="true">🖥️</div><p>Opcional. Agrega la Waveshare sin reemplazar la placa que ejecuta.</p><Button variant="outline" onClick={() => { onChangeCentralDisplay({ boardProfile: WAVESHARE }); setPairCredentials(null); setStep('pair-screen'); }}>Agregar Waveshare</Button></>}</article></div><ul className="board-guide-checks"><li>✓ Tiene acciones para ejecutar</li><li>✓ Los bloques y componentes son compatibles</li>{store.remote?.course && <li>✓ Pertenece al curso {store.remote.course.name}</li>}</ul><Button onClick={() => setStep(centralDisplay && !centralDisplay.hardwareId ? 'pair-screen' : 'connect')} disabled={board === WAVESHARE}>Continuar</Button>{board === WAVESHARE && <output className="account-help">Elegí una Wemos o DevKit como Placa del proyecto. La Waveshare se agrega aparte como Pantalla central.</output>}</section>}
