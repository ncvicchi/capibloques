...
export default function UsbBoard(props: { account: Account; store: AccountDraftStore; job: FirmwareJob | null; currentBoardProfile: BoardProfileId; currentFingerprint: string; onClose: () => void; onBuilds: () => void }) {
...
function UsbPanel({ account, store, job, currentBoardProfile, currentFingerprint, onClose, onBuilds, session }: { account: Account; store: AccountDraftStore; job: FirmwareJob | null; currentBoardProfile: BoardProfileId; currentFingerprint: string; onClose: () => void; onBuilds: () => void; session: UsbSession }) {
  ...
  const target = boardProfile(job?.boardProfile ?? currentBoardProfile);
  ...
  function cancel() {
    if (state.writingStarted && !window.confirm('¿Interrumpir la grabación? Puede quedar incompleta y la placa no arrancará hasta volver a grabar el firmware entero. No es Deshacer.')) return;
    session.cancel();
  }
  ...
  return <Dialog open onOpenChange={open => { if (!open) close(); }}><DialogContent className="firmware-dialog usb-dialog" showCloseButton={!busy && !monitoring}>
    <DialogHeader><DialogTitle>🔌 USB y monitor Serial</DialogTitle><DialogDescription>La placa se conecta por USB a esta PC, no al servidor. Chrome o Edge de escritorio · {target.name}.</DialogDescription></DialogHeader>
    ...
    <fieldset className="usb-checks" disabled={busy || monitoring || revoked}>
      ...
      <label><input type="checkbox" checked={identified} onChange={event => setIdentified(event.target.checked)} /> Identifiqué mi {target.shortName} y su cable USB de datos. No es otra placa.</label>
      ...
      {job && <label><input type="checkbox" checked={replace} onChange={event => setReplace(event.target.checked)} /> Quiero reemplazar el programa de la placa por esta versión. No hay Deshacer ni respaldo automático del firmware anterior.</label>}
    </fieldset>
    ...
    <div className="usb-actions">
      {!busy && !monitoring && <>
        {job && <Button disabled={!available || !identified || !safe || !replace || revoked} onClick={flash}>Elegir {target.shortName} y grabar</Button>}
    ...
    <p className="account-help">Cerrar USB o detener el simulador no detiene el programa físico. La placa funciona por su cuenta: desconectá su alimentación para detenerla con seguridad.</p>
