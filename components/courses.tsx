  ...
  return <>
    ...
    {listing && <>
      <main className="management-page courses-page" inert={locked} aria-hidden={locked || undefined}>
        ...
        <p className="account-help">La biblioteca y la revisión de proyectos se incorporarán en las próximas fases. Pertenecer a un curso todavía no comparte tus borradores del editor.</p>
      </main>
    ...
    <Dialog open={Boolean(draft || view)} onOpenChange={open => { if (!open) cancel(); }}><DialogContent className="management-dialog course-dialog" showCloseButton={!busy}>
      ...
      </fieldset></form> : view && <><p>{view.description || 'Este curso todavía no tiene descripción.'}</p>{view.members && <section><h2>Docentes y alumnos del curso</h2><ul className="course-member-list">{view.members.map(member => <li key={member.id}><div><strong>{member.displayName}</strong><span>@{member.alias} · {member.role}</span></div></li>)}</ul></section>}<p className="account-help">Todavía no se comparten proyectos desde esta pantalla.</p><Button className="account-action" onClick={cancel}>Cerrar curso</Button></>}
