...
export default function AccountDeletion({
  ...
  const headers = {
    ...
    'X-CSRFToken': token,
    'X-Capi-Account': actorId,
  };
  const valid = (ticket: number) =>
    active.current &&
    ...
    const refresh = async () => {
      ...
      try {
        const response = await fetch(url, {
          headers: { 'X-Capi-Account': actorId },
          cache: 'no-store',
          signal: AbortSignal.timeout(12000),
        });
        const data = (await response.json()) as Preview & { error?: string };
        if (!active.current || ticket !== generation.current) return;
        if (!response.ok || data.user?.id !== targetId)
          throw new Error(
            data.error ||
              'No pudimos preparar la baja. Cancelá y actualizá el listado.',
          );
        if (version.current && version.current !== data.version) {
          setReceipt('');
          setPermanent(false);
          setError(
            'La cuenta o sus proyectos cambiaron. Revisá los nuevos datos y prepará otro respaldo.',
          );
        }
  ...
  async function backup() {
    if (!preview?.canDelete || !privateBackup || locked || inFlight.current)
      return;
    const ticket = ++generation.current;
    ...
    try {
      ...
      if (hash !== checksum)
        throw new Error(
          'La descarga no coincide con el respaldo; no se habilitó la eliminación.',
        );
      const objectUrl = URL.createObjectURL(blob),
        link = document.createElement('a');
      link.href = objectUrl;
      link.download = `respaldo-cuenta-${targetId}.zip`;
      link.click();
      ...
      if (valid(ticket))
        setError(
          failure instanceof Error
            ? failure.message
            : 'Descarga interrumpida. No se eliminó la cuenta.',
        );
    } finally {
    ...
    try {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          confirmationAlias: alias,
          understandsLocalDrafts: localDrafts,
          understandsPermanent: permanent,
        }),
      ...
      if (!response.ok || data.deleted !== true)
        throw new Error(
          data.error ||
            'No se confirmó la baja. Cancelá y actualizá antes de repetir.',
        );
      deleted();
    } catch (failure) {
      if (valid(ticket)) {
        ...
        setError(
          failure instanceof Error
            ? failure.message
            : 'Resultado no confirmado. Cancelá y actualizá antes de repetir.',
        );
      }
    } finally {
      inFlight.current = false;
    ...
    >
      <AlertDialogContent className="management-dialog">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Esta baja elimina la cuenta y sus proyectos del servidor. No se
            puede deshacer; el ZIP permite importar los trabajos como proyectos
            nuevos.
          </AlertDialogDescription>
        </AlertDialogHeader>
        {error && (
        ...
        {!preview ? (
          <p>
            Verificando cuenta, proyectos y membresías… Si no hay conexión,
            cancelá y reintentá.
          </p>
        ) : (
          <>
            <p>
              <strong>{preview.user.displayName}</strong> (@{preview.user.alias}
              ) · {preview.user.isActive ? 'Activa' : 'Inactiva'}
            </p>
            <p>
            ...
            {!preview.canDelete ? (
              <p className="account-error">
                ...
                todas sus membresías en Gestionar cursos (incluidos los
                archivados). El último administrador activo está protegido.
              </p>
            ) : (
              <>
                <label htmlFor="backup-private" className="management-check">
                  <Checkbox
                    id="backup-private"
                    checked={privateBackup}
                    disabled={busy}
                    onCheckedChange={setPrivateBackup}
                  />
                  Entiendo que el respaldo incluye trabajos privados y lo
                  preparo para esta baja.
                </label>
                <Button
                  variant="outline"
                  disabled={busy || !privateBackup}
                  onClick={() => void backup()}
                >
                  Descargar respaldo ZIP
                </Button>
                {receipt && (
                  <output className="account-notice">
                    ZIP recibido y verificado. Comprobá que quedó guardado en
                    Descargas antes de confirmar. Vigencia para esta baja: 10
                    minutos.
                  </output>
                )}
                <p>
                ...
                <label htmlFor="backup-local" className="management-check">
                  <Checkbox
                    ...
                    checked={localDrafts}
                    disabled={busy}
                    onCheckedChange={setLocalDrafts}
                  />
                  Entiendo que los borradores locales no están respaldados y
                  debo pedir que los exporten antes de esta baja.
                </label>
                <label htmlFor="backup-permanent" className="management-check">
                  <Checkbox
                    id="backup-permanent"
                    checked={permanent}
                ...
                <label htmlFor="delete-alias">
                  Escribí el alias exacto para confirmar
                  <Input
                    id="delete-alias"
                    disabled={busy}
                    autoComplete="off"
