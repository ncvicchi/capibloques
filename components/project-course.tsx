...
export default function ProjectCourseDialog({
  project,
  accountId,
  token,
  store,
  ...
  useEffect(() => {
    ...
    void fetch('/api/projects/courses/', {
      ...
      .then(async (response) => {
        ...
        if (!response.ok || !Array.isArray(data.courses))
          throw new Error(
            'No pudimos verificar tus cursos. Cerrá y reintentá.',
          );
  ...
  async function save() {
    ...
    setError('');
    try {
      const response = await fetch(`/api/projects/${project.id}/course/`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
      ...
      if (!active.current || !store.active) return;
      if (!response.ok)
        throw new Error(data.error || 'No pudimos cambiar el curso.');
      saved();
      close();
    } catch (failure) {
      if (active.current)
        setError(
          failure instanceof Error
            ? failure.message
            : 'No pudimos confirmar el cambio. Cerrá y actualizá la biblioteca antes de repetir.',
        );
    } finally {
  ...
  return (
    ...
    >
      <DialogContent className="management-dialog" showCloseButton={!busy}>
        <DialogHeader>
          <DialogTitle>Elegir curso del proyecto</DialogTitle>
          <DialogDescription>
            {project.title}: se asigna la versión guardada en servidor, no los
            cambios pendientes del editor.
          </DialogDescription>
        </DialogHeader>
        {error && (
        ...
        {unavailable ? (
          ...
          <>
            <label className="library-filters" htmlFor="project-course-choice">
              Compartir con
              <select
                id="project-course-choice"
                disabled={busy || !courses}
                value={selected}
              ...
              >
                ...
                {project.course &&
                  !courses?.some(
                    (course) => course.id === project.course?.id,
                  ) && (
                    <option value={project.course.id}>
                      {project.course.name}
                    </option>
                  )}
                {courses?.map((course) => (
            ...
            {courses?.length === 0 && (
              <p>
                No tenés cursos activos como alumno. Podés conservar tu proyecto
                personal.
              </p>
            )}
            <p>
              Los docentes asignados al curso podrán ver y descargar este
              proyecto guardado. Los compañeros no. Volver a Personal revoca ese
              acceso, pero no borra copias que ya hayan descargado.
            </p>
          </>
        )}
        <div className="account-actions">
          ...
          <Button
            disabled={
              ...
              !courses ||
              Boolean(unavailable) ||
              selected === (project.course?.id ?? '')
            }
            onClick={() => void save()}
          >
            Guardar curso del proyecto
          </Button>
        </div>
      </DialogContent>
    </Dialog>
