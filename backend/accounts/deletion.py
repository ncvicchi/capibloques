...
def summary(target):
    ...
    version = salted_hmac(SALT, json.dumps([record(target)["version"], rows, members], default=str), algorithm="sha256").hexdigest()
    return {"user": record(target), "projects": {"count": len(rows), "active": sum(row[3] is None for row in rows), "trash": sum(row[3] is not None for row in rows), "bytes": sum(row[2] for row in rows)}, "memberships": len(members), "version": version,
...
def deletion(request, user_id):
    with access_lock():
        ...
        ProjectEvent.objects.bulk_create([ProjectEvent(actor=actor, actor_id_snapshot=actor.pk, project_id_snapshot=item.pk, revision=item.revision, action="account_deleted") for item in owned.only("id", "revision")])
...
def backup(request, user_id):
    ...
    with access_lock():
        ...
        if current["projects"]["count"] > 100 or current["projects"]["bytes"] > 50_000_000:
        ...
        try:
            ...
            with zipfile.ZipFile(archive, mode="w", compression=zipfile.ZIP_DEFLATED, compresslevel=1) as bundle:
                ...
                for project in target.projects.select_related("course").order_by("id").iterator(chunk_size=1):
                    ...
                    manifest["projects"].append({"id": str(project.pk), "title": project.title, "revision": project.revision, "file": filename, "sha256": hashlib.sha256(contents).hexdigest(), "trashedAt": project.trashed_at.isoformat() if project.trashed_at else None, "course": {"id": str(project.course_id), "name": project.course.name} if project.course_id else None})
                bundle.writestr("manifest.json", encoded(manifest))
                bundle.writestr("LEEME.txt", "Respaldo privado de baja de cuenta. No contiene contraseñas, sesiones, historial ni borradores locales. Para recuperar trabajos: crear una cuenta e importar por separado los JSON de projects/; obtendrán nueva identidad personal, sin compartir automáticamente con cursos. Guardar este ZIP en un lugar privado. No restaura automáticamente la cuenta original.\n")
            archive.seek(0)
