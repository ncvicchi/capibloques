...
- 20: **terminada en software y publicada en `main` como `6dece1e`**; el recorrido infantil es Usar en placa → Conectar → Preparar firmware CapiBloques cuando haga falta → Enviar reglas y ejecutar. Arduino/ESP-IDF, compilación específica y monitor Serial se conservan bajo Herramientas avanzadas para adultos. DEV permanece en `8103b14` hasta la actualización del propietario y falta aceptación física; ver [FASE_20_USO_GUIADO_PLACA.md](FASE_20_USO_GUIADO_PLACA.md).
