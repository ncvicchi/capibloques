...
- 20: **asistente base corregido en `9914ae8`; selección visual reabierta y pendiente**. El recorrido infantil usa una pantalla por decisión: revisar → conectar → preparar sólo si hace falta → Wi-Fi sólo si corresponde → enviar → ejecutar. Debe mostrar «Placa actual» con foto y permitir cambiarla en cualquier momento, también desde el asistente, ofreciendo sólo destinos compatibles y sin reasignar/borrar silenciosamente. Arduino/ESP-IDF, compilación específica y monitor Serial se conservan bajo Herramientas avanzadas para adultos. DEV permanece en `8103b14` hasta la actualización del propietario y falta aceptación física; ver [FASE_20_USO_GUIADO_PLACA.md](FASE_20_USO_GUIADO_PLACA.md).
