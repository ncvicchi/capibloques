...
- 20: **terminada en software, con corrección de asistente en curso el 25 de septiembre**; el recorrido infantil usa una pantalla por decisión: revisar → conectar → preparar sólo si hace falta → Wi-Fi sólo si corresponde → enviar → ejecutar. Arduino/ESP-IDF, compilación específica y monitor Serial se conservan bajo Herramientas avanzadas para adultos. La base publicada era `6dece1e`; registrar aquí la revisión final de la corrección. DEV permanece en `8103b14` hasta la actualización del propietario y falta aceptación física; ver [FASE_20_USO_GUIADO_PLACA.md](FASE_20_USO_GUIADO_PLACA.md).
