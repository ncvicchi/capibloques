...
- 18: **reabierta/en curso**; hay dos modos. Waveshare principal ejecuta una simulación virtual sin GPIO: el intérprete 1.5.4 inicializa el RGB 800 × 480, dibuja posiciones/nombres de escena y actualiza semáforo, LED, servo, motor, robot/Otto y entradas binarias mediante una tarea cooperativa. La placa real en `COM12` confirmó comunicación, ejecución, telemetría y escena RGB; quedan glitches visuales, touch y fidelidad completa del catálogo. Como Pantalla central Wi‑Fi acompaña a una Wemos/NodeMCU/DevKit; esa escena remota continúa pendiente. Ver [FASE_18_CONTROLES_LOCALES.md](FASE_18_CONTROLES_LOCALES.md).
...
- 20: **asistente base, selección y pareja Waveshare implementados en software**. Distingue Waveshare principal virtual y Pantalla central opcional. Con 1.5.4, `RUN` inicia también el espejo web a 1× y minimiza el asistente sin cerrar Web Serial. La barra principal distingue **Simulador web** de **Placa**, controla Pausar/Reanudar/Detener en ambos destinos y muestra la telemetría física traducida en **Mensajes de placa**; el registro usa nombres de escena y resultados concretos, no IDs internos. Quedan escena/touch remotos e historial global del cambio. Ver [FASE_20_USO_GUIADO_PLACA.md](FASE_20_USO_GUIADO_PLACA.md).
...
- 36: **runtime 1.5.4 implementado, publicado y probado físicamente en Waveshare**. `CapiRules` v1/ABI 1, `CapiLink`, commit A/B, arranque autónomo, telemetría, renderer RGB cooperativo y PSRAM octal están entregados. La Waveshare real en `COM12` completó el recorrido hasta `program-done` y mostró la escena; quedan glitches visuales y aceptación física de las otras placas/componentes. La UI traduce telemetría a acciones amigables y nombres configurados, sin exponer `blockId`. Plan/estado en [FASE_36_FIRMWARE_INTERPRETE.md](FASE_36_FIRMWARE_INTERPRETE.md).
