...
- 18: **reabierta/en curso**; el tablero lógico entregado es una base. La Waveshare debe ser Pantalla central Wi‑Fi de una Wemos/NodeMCU/DevKit que ejecuta el programa: dibuja la misma escena, refleja componentes reales y virtuales y envía entradas autorizadas. Falta implementar este modo, DEV y aceptación física; ver [FASE_18_CONTROLES_LOCALES.md](FASE_18_CONTROLES_LOCALES.md).
...
- 20: **asistente base `9914ae8`; selección visual Wemos/DIYmall publicada en `1904e76`**. Muestra «Placa actual» con foto, previsualiza las conexiones que cambiarán y confirma antes de modificar escena/perfil. Queda ampliar el modelo a Placa del proyecto obligatoria + Pantalla central Waveshare opcional: al activarla se muestran las dos y CapiBloques configura automáticamente en ambas `WSMMMMMM` —seis últimos dígitos de la MAC de la Waveshare—, contraseña única e identidad de pareja; el alumno no elige Wi-Fi. Waveshare alterna frente/dorso, pero está deshabilitada porque su intérprete de panel/touch aún no existe. Quedan además historial global del cambio y aceptación física. Arduino/ESP-IDF, compilación específica y monitor Serial permanecen avanzados. DEV sigue en `8103b14`; ver [FASE_20_USO_GUIADO_PLACA.md](FASE_20_USO_GUIADO_PLACA.md).
...
- 34: pendiente; Pantalla central Wi‑Fi y servicios tipados. Activarla agrega una segunda placa sin sustituir la Placa del proyecto. Cada Waveshare mantiene un AP único/persistente `WSMMMMMM`, derivado de los últimos seis dígitos de su MAC; CapiBloques configura automáticamente en ambas placas la red, clave única e identidad, sin elección del alumno y sin guardar secretos en el proyecto/servidor. La ejecutora envía telemetría y sigue autónoma al desconectarse; la Waveshare refleja la escena y envía entradas/solicitudes publicadas, nunca GPIO/código arbitrario. Ver [FASE_34_PANTALLA_CENTRAL_WIFI.md](FASE_34_PANTALLA_CENTRAL_WIFI.md).
...
- 36: **terminada en software y desplegada en DEV el 24 de septiembre de 2026**. `CapiRules` v1/ABI 1 binario y acotado, `CapiLink`, actualización Web Serial, commit A/B posterior a verificación, arranque autónomo, telemetría y generación reproducible de artefactos fuera de Git. Los intérpretes 1.4.0 de Wemos y DIYmall S3 fueron compilados, empaquetados y publicados durante la actualización `8103b14`; falta ensayar físicamente instalación, reinicio, reglas y controladores. Plan/estado en [FASE_36_FIRMWARE_INTERPRETE.md](FASE_36_FIRMWARE_INTERPRETE.md).
