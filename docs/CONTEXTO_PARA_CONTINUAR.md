...
- 18: **reabierta/en curso**; hay dos modos. Waveshare principal ejecuta una simulación virtual sin GPIO: el intérprete 1.5.4 inicializa el RGB 800 × 480, dibuja posiciones/nombres de escena y actualiza semáforo, LED, servo, motor, robot/Otto y entradas binarias mediante una tarea cooperativa. Falta compilar/publicar en DEV y aceptar físicamente colores, estabilidad y estados; el touch y la fidelidad completa del catálogo siguen pendientes. Como Pantalla central Wi‑Fi acompaña a una Wemos/NodeMCU/DevKit; esa escena remota continúa pendiente. Ver [FASE_18_CONTROLES_LOCALES.md](FASE_18_CONTROLES_LOCALES.md).
...
- 36: **runtime 1.5.4 implementado localmente; publicación DEV y aceptación física pendientes**. `CapiRules` v1/ABI 1, `CapiLink`, commit A/B, arranque autónomo y telemetría están entregados. La Waveshare real confirmó con 1.5.3 en `COM12` el recorrido completo hasta `program-done`; 1.5.4 agrega el renderer RGB cooperativo y PSRAM octal. La UI traduce telemetría a acciones amigables y nombres configurados, sin exponer `blockId`. Pasaron typecheck, lint y smoke; falta construir los tres artefactos en DEV, grabar 1.5.4 y confirmar pantalla/estabilidad. Plan/estado en [FASE_36_FIRMWARE_INTERPRETE.md](FASE_36_FIRMWARE_INTERPRETE.md).
