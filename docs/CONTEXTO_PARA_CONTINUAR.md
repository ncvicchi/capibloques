...
- El propietario autorizó ejecutar sin interrupciones las fases 31, 32, 33 y 37, con cierre y commit por fase. Fases 11–16, 27–31 y 36 fueron ejecutadas en software; también autorizó específicamente la Matriz LED de fase 23. La autorización vigente no abre 34, 35 ni otras fases.
...
- 36: **terminada en software el 24 de septiembre de 2026; despliegue en DEV y aceptación física pendientes**. `CapiRules` v1/ABI 1 binario y acotado, `CapiLink`, selector Simulador/Placa, actualización Web Serial, commit A/B posterior a verificación, arranque autónomo, telemetría y generación reproducible de artefactos fuera de Git. La negociación bloquea firmware viejo, placa/ABI incorrecta y capacidades ausentes. El runtime requerido por la web es ahora 1.3.0 e incorpora `component-state` y `wifi-messages`; además declara núcleo, paralelo/fork-join, expresiones/variables/temporizadores, entradas analógicas/digitales, GPIO, LED, semáforo, motores/robot, servo, buzzers, matriz MAX7219 cooperativa, Mensajes con trama protegida, Otto genérico, LCD 16×2/20×4, SSD1306 e ILI9341/ILI9488, Wi‑Fi AP/cliente con aprovisionamiento USB privado, contador y consola. Falta actualizar DEV, recompilar/publicar 1.3.0 y ensayar físicamente instalación, reinicio, reglas y controladores. Plan/estado en [FASE_36_FIRMWARE_INTERPRETE.md](FASE_36_FIRMWARE_INTERPRETE.md).
