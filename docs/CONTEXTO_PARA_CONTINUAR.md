...
- 36: **autorizada y en curso**. Primera entrega software: `CapiRules` v1/ABI 1 binario y acotado, `CapiLink`, selector Simulador/Placa, actualización Web Serial, commit persistente posterior a verificación, arranque autónomo, telemetría, runtime base Wemos/S3 y generación reproducible de artefactos fuera de Git. La negociación bloquea firmware viejo, placa/ABI incorrecta y capacidades ausentes. El runtime 1.0 sólo declara núcleo, GPIO, LED, semáforo, motores/robot, contador, consola y entradas digitales; controladores avanzados y aceptación física siguen pendientes. Plan/estado en [FASE_36_FIRMWARE_INTERPRETE.md](FASE_36_FIRMWARE_INTERPRETE.md).
