...
- 20: **terminada en software y corregida en `9914ae8`**; el recorrido infantil usa una pantalla por decisión: revisar → conectar → preparar sólo si hace falta → Wi-Fi sólo si corresponde → enviar → ejecutar. Arduino/ESP-IDF, compilación específica y monitor Serial se conservan bajo Herramientas avanzadas para adultos. Typecheck, lint, smoke, build y 7/7 recorridos Chromium pasaron. DEV permanece en `8103b14` hasta la actualización del propietario y falta aceptación física; ver [FASE_20_USO_GUIADO_PLACA.md](FASE_20_USO_GUIADO_PLACA.md).
