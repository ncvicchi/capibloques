...
Estado: **fases 0 a 9 implementadas y verificadas en DEV** (8 de septiembre de 2026). La fase 4 incluye autoguardado, recuperación local, salida compartida, continuidad durante cortes, borradores de escena, historial restaurable y purga individual confirmada. Alcance y pruebas en [fase 4 completa](FASE_4_RECUPERACION_E_HISTORIAL.md). La [fase 5 completa](FASE_5_SUPERVISION_DOCENTE.md) incorpora revisión de sólo lectura, simulación, copias y devoluciones sobre versiones protegidas. La [fase 6 completa](FASE_6_EXPERIENCIA_Y_PROGRAMACION.md) agrega avatares, favoritos, inicio único, caminos en paralelo y ejecución visible/guiada. La [fase 7 completa](FASE_7_MENSAJES_Y_PANTALLAS.md) agrega mensajes a una pantalla LCD/OLED/TFT por proyecto y zonas de texto. La [fase 8 completa](FASE_8_ARDUINO_Y_ESP_IDF.md) conserva Arduino y agrega ZIP ESP-IDF nativo. La [fase 9](FASE_9_COMPILACION_Y_DESCARGA.md) añade cola durable, compiladores aislados, firmware completo privado, configuración administrativa y Wi-Fi por compilación privada autorizada. No se probó hardware físico. El propietario pidió avanzar por fases completas, no por subfases. La [fase 10](FASE_10_USB_Y_SERIAL.md) tiene software implementado en DEV y aceptación física pendiente porque no hay Wemos disponible. Las fases 11 y 12 se entregaron consecutivamente: el [contexto vivo de fase 11](CONTEXTO_PARA_CONTINUAR.md) queda documentado y la [UX de fase 12](FASE_12_MESA_DE_TRABAJO.md) verificada en DEV y CI, con evidencia de cierre en su guía. Se actualizará ese contexto al cerrar cada fase solicitada. La nueva [fase 13](FASE_13_ACCESO_EXTERNO_DEV.md) planifica acceso externo persistente y seguro a DEV; las fases 13–18 siguen pendientes de implementación y autorización de ejecución. Producción se renombra **Fase final** y queda **postergada**.
Fecha de actualización: 12 de septiembre de 2026.
Backlog posterior: [pedidos completos](BACKLOG.md), asignados a las [fases 12–18](PLAN_FASES_BACKLOG.md), precedidas por la fase 11 de contexto portable. La fase 12 está entregada y verificada en DEV/CI; los demás pedidos no amplían esa autorización.
...
- Servidores públicos estables; nunca `runserver`, `vinext dev` ni HMR expuestos a Internet. La fase 13 reutilizará la VM Nginx del propietario para el subdominio HTTPS de DEV y la Fase final hará lo propio, con configuración y secretos separados, para producción. No instalar otro proxy público por defecto ni confundir la VM Nginx con el host Proxmox.
...
Numeración vigente: fases consecutivas 0–18, sin letras, más **Fase final** de producción, sin número y postergada. Las fases 0–9 están implementadas y verificadas en DEV. La fase 10 está autorizada y tiene implementación USB/monitor en DEV, pero no se cierra sin probar físicamente una Wemos identificada. Las fases 11 y 12 están entregadas: contexto portable documentado y UX verificada en DEV/CI; evidencia en la guía de fase 12. La nueva fase 13 publica DEV de manera controlada y desplazó las anteriores 13–17 a 14–18. Las fases 13–18 necesitan autorización de ejecución. Los nombres de las guías históricas se conservan para mantener sus enlaces; no son fases independientes del plan actual.
...
| 13. Acceso externo a DEV · planificada | Subdominio HTTPS, runtime persistente, proxy Nginx y origen interno restringido | Acceso externo real en Chrome/Edge, servicio tras reinicio, certificado renovable, mismo origen y sin publicar HMR, base, Docker, compilador o administración; DNS creado por el propietario. |
...
2. Los accesos de ambas VMs por gateway ya se probaron y permanecen fuera de Git; para fase 13, confirmar FQDN, destino DNS/certificado y acceso autorizado a la VM Nginx exacta. El propietario crea el registro en GoDaddy sin compartir credenciales. Autenticación GitHub realizada por el propietario tras aviso, sin enviar secretos al chat.
...
- Usar la VM para backend, base y herramientas de desarrollo, con datos ficticios y configuración separada de producción. Hasta fase 13, permitir acceso sólo por LAN/túnel. En fase 13 se publicará únicamente un runtime estable detrás del HTTPS y la barrera de acceso de la VM Nginx; nunca el servidor con HMR ni los servicios internos directamente a Internet.
