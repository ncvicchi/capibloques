...
Estado: fases 0A, 0B, 1, 2A, 2B.1, 2B.2, 2B.3, 3A, 3B, 4A y 4B.1 entregadas. **4B.2a: salida en computadoras compartidas** implementada en DEV; alcance, límites y validación en [fase 4B.2a](FASE_4B2A_SALIDA.md). Amplía la [recuperación 4B.1](FASE_4B1_RECUPERACION.md) con conservar/exportar/quitar copias locales al salir. No completa toda 4B.2 ni la fase 4: no hay historial ni modo offline. **Siguiente subfase, pendiente de validación y otro OK: 4B.2b, comportamiento ante desconexión.** La revisión docente visual con devoluciones sigue en fase 5.
Fecha de actualización: 7 de septiembre de 2026.
Actualización: ABM en `/gestion/usuarios/`, marca institucional en `/gestion/colegio/`, cursos en `/gestion/cursos/` y biblioteca personal en el editor → Mis proyectos. Guardado manual y automático del proyecto vinculado en PostgreSQL con revisión optimista, importación/exportación y papelera recuperable. IndexedDB conserva documento y operación de Guardar antes de la red, con recuperación tras cierre y listado de otras copias de la cuenta. Al salir, elección explícita de conservar o retirar copias locales después de confirmar logout; no se limpian otras cuentas, archivos descargados ni equipos remotos. No es una cola general ni un respaldo; sigue siendo necesaria una sesión verificada. Producción no activada; Arduino + ESP-IDF, binarios/carga USB y concurrencia configurable permanecen en sus fases pendientes.
...
| 3A. Biblioteca personal | Guardado manual en servidor, búsqueda, abrir/renombrar/duplicar, JSON y papelera recuperable | Recuperación independiente del borrador local, sólo propietario, validación y conflictos sin sobrescritura; probar antes de avanzar. |
| 3B. Proyectos de curso y bajas | Vínculo explícito a curso autorizado y baja de cuentas con conteo/respaldo/confirmación | Se preserva privacidad por objeto, retiradas/archivo de cursos y bajas no borran datos por cascada incidental; sin supervisión completa aún. |
| 4A. Autoguardado acotado | Envíos del proyecto vinculado después del primer Guardar, opción por cuenta/navegador, reintentos en memoria y conflictos visibles | Agrupa cambios sin bloquear edición; no crea proyectos implícitos, no pierde ediciones durante el envío ni publica borradores de escena; respeta sesión y revisión. No promete recuperación al cerrar. |
| 4B.1. Recuperación durable acotada | Documento y operación de Guardar en IndexedDB, recuperación tras cierre, varios borradores y conservar al reemplazar | Pasan respuesta perdida y reinicio, abortos locales, múltiples pestañas/proyectos, cuotas sin expulsión, exportar/quitar confirmado y cambio de cuenta. No promete apertura offline ni limpieza al salir. |
| 4B.2. Desconexión y equipos compartidos | Política explícita para conservar o retirar datos locales al salir; comportamiento del editor ya cargado ante cortes | Resolver sin perder pendientes ni filtrar datos entre cuentas; no confundir disponibilidad offline con autorización vigente ni debilitar revocación. |
| 4C. Historial y escena | Revisiones restaurables, borradores de escena y política de papelera/purga | Restaurar crea una revisión nueva; Cancelar no publica la escena, cierre permite recuperarla y purga tiene límites y confirmaciones verificadas. |
...
División para entregas acotadas: 3A incorpora la biblioteca personal; 3B agrega compartir por curso y baja administrativa preparada con respaldo. No hay purga automática ni borrado definitivo individual: la papelera cuenta contra cuotas. Desactivar sigue conservando todos los trabajos. El ZIP de 3B reemplaza la ficha de 2B.1 para preparar bajas y recupera trabajos mediante importación, no la cuenta original ni borradores locales. La fase 4 se divide en 4A, 4B.1, 4B.2 y 4C, con validación y nuevo OK entre entregas. Para mantener la siguiente entrega acotada, 4B.2 se entrega en **4B.2a salida compartida** y **4B.2b desconexión**: no se cuenta 4B.2 como completa sólo por entregar la primera. Las garantías completas de recuperación de las secciones anteriores son el objetivo de toda la fase, no lo entregado en una sola subfase. La fase 5 permanece separada: listar/descargar JSON no se presenta como supervisión completa.
