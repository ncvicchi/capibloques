...
Estado: **fases 0 a 8 implementadas y verificadas en DEV** (7 de septiembre de 2026). La fase 4 incluye autoguardado, recuperación local, salida compartida, continuidad durante cortes, borradores de escena, historial restaurable y purga individual confirmada. Alcance y pruebas en [fase 4 completa](FASE_4_RECUPERACION_E_HISTORIAL.md). La [fase 5 completa](FASE_5_SUPERVISION_DOCENTE.md) incorpora revisión de sólo lectura, simulación, copias y devoluciones sobre versiones protegidas. La [fase 6 completa](FASE_6_EXPERIENCIA_Y_PROGRAMACION.md) agrega avatares, favoritos, inicio único, caminos en paralelo y ejecución visible/guiada. La [fase 7 completa](FASE_7_MENSAJES_Y_PANTALLAS.md) agrega mensajes a una pantalla LCD/OLED/TFT por proyecto y zonas de texto. La [fase 8 completa](FASE_8_ARDUINO_Y_ESP_IDF.md) conserva Arduino y agrega ZIP ESP-IDF nativo, con los siete circuitos compilados en ambos frameworks y descargas verificadas en DEV. No se probó hardware físico. El propietario pidió avanzar por fases completas, no por subfases. Las fases 9–11 siguen pendientes de autorización.
...
Estado: las dos salidas de fuentes están implementadas y compiladas en la fase 8. Compilación en servidor/descarga de firmware y USB desde la web siguen pendientes (fases 9 y 10); no se validó en placa. El recorrido es simular comportamiento en la web, elegir salida, compilar, descargar o grabar y ejecutar autónomamente. Se conservan Arduino y exportación JSON; no se necesita controlar la placa en vivo.
