...
| 12. Editor despejado · planificada | Encabezados compactos, catálogo separado y sesión sin interrupciones por foco | Se recupera espacio útil, arrastrar/editar funciona en Chrome/Edge y cambiar de ventana no dispara validaciones por foco ni interrumpe el editor; se conservan permisos y trabajo. |
...
| 14. TX/RX serial · planificada | UART configurable, mensajes y bifurcación igual/distinto | Simulación, Arduino e IDF coinciden ante mensajes completos, ausencia, timeout y fragmentos; espera no bloqueante y ensayo físico en Wemos. |
| 15. ESP32-S3 DevKit · planificada | Perfiles de placa y recorrido completo hasta USB | Modelo exacto, pines y memoria validados; ambas salidas compilan y se graban/ejecutan en la DevKit identificada; Wemos conserva compatibilidad. |
| 16. Waveshare de 5 pulgadas · planificada | Perfil de placa, pantalla y entrada táctil si existe | Modelo/revisión identificados y mensajes/zonas funcionando físicamente con ambas herramientas, límites medidos y USB validado; no implica aún escena interactiva. |
