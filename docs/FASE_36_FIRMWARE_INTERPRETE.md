...
La compilación fijada en ESP-IDF 5.5.5 produjo y empaquetó correctamente los intérpretes anteriores de Wemos D1 R32 y ESP32-S3 en CI el 24 de septiembre de 2026. La versión 1.5.2 incluye el tercer perfil Waveshare y aún debe compilarse/publicarse en DEV. La prueba física de 1.5.0 reveló el watchdog de `capi-matrix`; 1.5.1 lo corrigió, pero no recibía CapiLink por el USB nativo usado para grabar la Waveshare. Ambas quedan obsoletas y la web exigirá 1.5.2. La fase **no tiene aún aceptación física completa**: faltan confirmar 1.5.2 por USB, carga de reglas, emparejamiento y controladores con las placas reales. El firmware Waveshare sólo cubre por ahora identidad/AP; render RGB y touch pertenecen a fases 18/34. Arduino y ESP-IDF por proyecto siguen disponibles.
