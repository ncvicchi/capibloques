...
Alias elegido por el propietario: `administrador`; nombre visible: `Administrador`. Se almacena el alias en minúsculas, y se puede ingresar usando mayúsculas. **No se asigna ni publica una contraseña predeterminada.** Hasta ejecutar este paso no existe la primera cuenta en DEV; los tests utilizan una base separada y no dejan cuentas reales.
...
- DEV sigue con HTTP sólo en loopback por SSH. No habilitar acceso público sin HTTPS, cookies Secure, segundo factor administrativo y configuración de producción de las fases pendientes.
...
Se entregan cambios con commit/push y se detiene aquí. Próximo punto de validación del propietario: crear `administrador`, ingresar y probar Guardar/Cancelar/cierre antes de autorizar 2B.
