...
- Corrida completa de colegio: **20/20 en Chrome y Edge**, sin reintentos, incluido un caso de API real por navegador. Después del último ajuste de navegación en Mi cuenta: **18/18 de regresión** de cuenta, marca pública y pantallas angostas, con dos casos de API real por navegador.
- Revisión visual del panel y Mi cuenta; navegación administrativa separada y comprobada a 390 px con fuente raíz ampliada. Las capturas usan respuestas ficticias interceptadas, no modifican los datos reales de DEV.
- La prueba inicial de recarga esperaba incorrectamente que finalizara una navegación cancelada por `beforeunload`; agotó el tiempo aunque la captura mostraba el borrador intacto. Se corrigió el disparador del ensayo y pasó en ambos navegadores, sin aumentar timeouts ni reintentos. No se eliminó la comprobación.
- **Cierre del código `5de89aa`: CI completo aprobado**, con 66 pruebas backend, 42 de interfaz, typecheck, lint, smoke, auditoría de dependencias, dos compilaciones Wemos y build estático. [Ejecución final](https://github.com/ncvicchi/capibloques/actions/runs/34066785359). DEV actualizado, API y frontend saludables, sin nuevos reinicios del editor durante las pruebas. El commit posterior sólo registra esta evidencia.

La siguiente subfase es **2B.3: cursos y membresías**, pendiente de nuevo OK. Biblioteca de proyectos, supervisión pedagógica, avatares, favoritos, ESP-IDF, compilación y USB conservan su lugar en el plan.
