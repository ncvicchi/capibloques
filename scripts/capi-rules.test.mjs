...
const pair = createPairCredentials({ protocol: 'CapiLink', firmware: '1.5.4', abi: 1, board: 'waveshare-esp32-s3-touch-lcd-5-28117', hardwareId: 'A1B2C33FA21C', maxRulesBytes: 32768, capabilities: ['pairing'], resources: { pwmChannels: 8 } });
...
const writable = new WritableStream({ write(bytes) {
  ...
  const reply = packet.type === 'HELLO'
    ? { type: 'HELLO', protocol: 'CapiLink', firmware: '1.5.4', abi: 1, board: 'waveshare-esp32-s3-touch-lcd-5-28117', maxRulesBytes: 32768, capabilities: ['core', 'counter', 'serial'], resources: { pwmChannels: 8 } }
