...
import { registerBlocks, ensureSingleStart, compileWorkspace, workspaceDevices,
  workspaceBoardProfiles, serializedDeviceIds, serializedAreaIds, collectSerializedDeviceIds, refreshDeviceFields } from '../lib/blockly-engine.ts';
...
export function generateBuild(document, framework, wifi = null) {
  ...
  workspaceBoardProfiles.set(workspace, project.target.boardProfile);
  ...
  try {
    ...
    result = generateEsp32CodeResult(compileWorkspace(workspace), project.metadata.title, project.scene, framework, project.target.boardProfile);
  ...
  return { files, usesWifi, program: result.program, target: project.target };
}
...
if (process.argv[1]?.endsWith('compiler-generate.mjs')) {
  ...
  try {
    ...
    const { files, usesWifi, target } = generateBuild(data.document, data.framework, data.wifi);
    process.stdout.write(JSON.stringify({ files, usesWifi, target }));
  } catch {
