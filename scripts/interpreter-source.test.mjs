...
for (const operation of ['servo', 'buzzer', 'tone', 'matrixScroll', 'visualWait', 'messageSend', 'messageReceiveWait'])
  assert.match(source, new RegExp(`"${operation}"`), `missing interpreter operation ${operation}`);
for (const capability of ['parallel', 'variables', 'servo', 'buzzer', 'matrix', 'messages'])
