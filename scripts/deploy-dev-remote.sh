...
for profile in profiles:
    try:
        ...
        assert info["sourceRevision"] == revision and info["version"] == "1.1.0" and bundle.is_file()
    except Exception:
        raise SystemExit(1)
PY
then
...
if ((BUILD_INTERPRETER)); then
  ...
  docker run --rm --cpus 1 --memory 1200m --memory-swap 1500m \
    ...
    bash -lc '. "$IDF_PATH/export.sh" >/dev/null && ./scripts/build-interpreter-firmware.sh 1.1.0 /project/public/interpreter '"$INTERPRETER_SOURCE"
...
for profile in ("wemos-d1-r32", "diymall-esp32-s3-devkitc-v1-n16r8"):
    info = json.loads((root / f"{profile}.json").read_text(encoding="utf-8"))
    bundle = root / info["bundle"]
    assert info["sourceRevision"] == revision and info["version"] == "1.1.0"
