...
test('permite elegir simulador o placa sin confundir sus controles', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Placas del proyecto' })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Placa del proyecto' })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Pantalla central' })).toBeVisible();
  await expect(page.getByRole('img', { name: /Wemos D1 R32, vista superior/i })).toBeVisible();
  await expect(page.getByText('Paso 1 de 4 · Revisar las placas')).toBeVisible();
  await expect(page.getByRole('button', { name: 'Conectar mi placa' })).toHaveCount(0);
  await page.getByRole('button', { name: 'Agregar Waveshare' }).click();
  await expect(page.getByRole('heading', { name: 'Primero, conectá la pantalla central' })).toBeVisible();
  await expect(page.getByText(/No tendrás que elegir una red ni escribir una contraseña/i)).toBeVisible();
  await expect(page.getByText('vista frontal')).toBeVisible();
  await expect(page.getByText('vista trasera')).toBeVisible({ timeout: 3000 });
  await page.getByRole('button', { name: 'Atrás' }).click();
  await page.getByRole('button', { name: 'Quitar pantalla' }).click();
  await page.getByRole('button', { name: 'Cambiar placa' }).click();
  await expect(page.getByRole('heading', { name: '¿Qué placa ejecutará el programa?' })).toBeVisible();
  await expect(page.locator('.board-choice').filter({ hasText: 'Waveshare 5″' })).toHaveCount(0);
