...
export async function mockLibrary(page: Page, existing?: Awaited<ReturnType<typeof createLibraryState>>) {
  ...
  await page.route('**/api/projects/**', async route => {
    ...
    if (req.method() === 'GET') {
      if (id === 'courses') return route.fulfill({ json: { courses: control.courses } });
    ...
    if (!id) {
      ...
      else if (action === 'course') saved.project.course = body.courseId ? { ...control.courses.find(course => course.id === body.courseId)!, isArchived: false, ownerCanEdit: true } : null;
