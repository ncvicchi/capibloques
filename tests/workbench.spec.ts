...
test('catálogo separado: fondo opaco, cierre explícito y Escape, arrastre funcional', async ({ page }) => {
  await open(page);
  await page.getByRole('treeitem', { name: /Tiempo/ }).click();
  await expect(page.getByRole('button', { name: 'Cerrar catálogo de bloques' })).toBeVisible();
  const opacity = await page.locator('.blocklyFlyoutBackground').first().evaluate(element => getComputedStyle(element).fillOpacity);
  expect(opacity).toBe('1');
  await page.getByRole('button', { name: 'Cerrar catálogo de bloques' }).click();
  await expect(page.locator('.blocklyFlyout')).not.toBeVisible();
  await page.getByRole('treeitem', { name: /Tiempo/ }).click(); await page.keyboard.press('Escape');
  await expect(page.locator('.blocklyFlyout')).not.toBeVisible();
  await page.getByRole('treeitem', { name: /Tiempo/ }).click();
  const block = page.locator('.blocklyFlyout .blocklyDraggable').first(), box = (await block.boundingBox())!;
