...
El editor exige ingreso con alias y contraseña. En la VM de desarrollo corren Django + PostgreSQL, el [acceso y sesiones](docs/FASE_2A_ACCESO.md) en `/cuenta/` y el [ABM de usuarios](docs/FASE_2B1_USUARIOS.md) en `/gestion/usuarios/`, exclusivo del administrador. Cursos, membresías y proyectos personales se guardan en servidor. La [biblioteca de fase 3](docs/FASE_3A_BIBLIOTECA.md) se abre con **Mis proyectos** en el editor. El alojamiento elegido es Proxmox; GitHub Pages deja de utilizarse. El frontend permite exportación estática, pero necesita la API del mismo origen. La [fase 4](docs/FASE_4_RECUPERACION_E_HISTORIAL.md) incluye autoguardado, recuperación local del proyecto y su envío pendiente, salida en equipos compartidos, continuidad durante cortes, recuperación de escenas e historial restaurable. La [fase 5](docs/FASE_5_SUPERVISION_DOCENTE.md) incorpora revisión docente, simulación de sólo lectura y devoluciones por versión. La [fase 6](docs/FASE_6_EXPERIENCIA_Y_PROGRAMACION.md) agrega avatares, favoritos, inicio único, caminos en paralelo y ejecución guiada visible. La [fase 7](docs/FASE_7_MENSAJES_Y_PANTALLAS.md) añade mensajes en una pantalla LCD/OLED/TFT. La [fase 8](docs/FASE_8_ARDUINO_Y_ESP_IDF.md) conserva Arduino y agrega ZIP ESP-IDF nativo. La [fase 9](docs/FASE_9_COMPILACION_Y_DESCARGA.md) incorpora compilación aislada en servidor, cola, descarga de firmware y configuración Wi-Fi privada. La [fase 10](docs/FASE_10_USB_Y_SERIAL.md) incorpora grabación Web Serial y monitor real en DEV; su aceptación física en Wemos sigue pendiente. Producción pasa a ser la **Fase final, postergada**. El [contexto para continuar](docs/CONTEXTO_PARA_CONTINUAR.md) es la documentación viva de fase 11 y se actualiza en cada entrega. Las fases 11 y 12 están entregadas. La [fase 12](docs/FASE_12_MESA_DE_TRABAJO.md) incluye la mesa de trabajo compacta, cámara de escena, guía visual Wemos y sesión sin consultas por foco; verificada en DEV y CI, con resultados en esa guía. La nueva [fase 13](docs/FASE_13_ACCESO_EXTERNO_DEV.md) planifica acceso externo HTTPS y persistente a DEV mediante la VM Nginx existente y DNS administrado por el propietario. Las [fases 13–18](docs/PLAN_FASES_BACKLOG.md) siguen pendientes de implementación y autorización de ejecución.
...
El [backlog](docs/BACKLOG.md) distingue lo implementado en fase 12 de lo pendiente: ejecución visual dentro de bloques, paralelo vertical, perfiles ESP32-S3 DevKit/Waveshare de 5 pulgadas, display interactivo y TX/RX serial con bifurcaciones.
...
Límites del servidor: 100 proyectos y 50 MB actuales por cuenta, incluida la papelera, más 50 MB de historial y 10 MB de contenido de devoluciones/respuestas. Se conservan hasta 20 versiones recientes no protegidas, además de las comentadas; puntos manuales y automáticos cada cinco minutos. Hasta 50 devoluciones por proyecto y 1000 caracteres por mensaje/respuesta. Un proyecto comentado conserva su curso; para otro contexto se hace una copia. Papelera protegida 30 días; luego purga individual confirmada desde Historial, sin cron automático en DEV. La purga elimina también historial y conversaciones, con advertencia explícita. Recuperación local: 30 copias y 50 MiB por cuenta/origen, sin expulsión automática; el navegador puede imponer un límite menor. Los cambios se agrupan durante 350 ms y el almacenamiento puede perderse: no sustituye un respaldo. Restaurar una versión crea una revisión nueva en el servidor, sin reemplazar el editor abierto.

La baja administrativa de una cuenta inactiva y sin membresías exige conteo, respaldo ZIP vigente de sus trabajos y confirmaciones. Desde [fase 5](docs/FASE_5_SUPERVISION_DOCENTE.md), incluye papelera, historial y conversaciones en archivos separados con hashes; no contraseñas ni borradores locales. Recuperar implica importar los JSON como proyectos nuevos y consultar las conversaciones por separado, no restaurar la cuenta original. Borrar un docente anonimiza su autor en devoluciones de trabajos ajenos, sin borrar esos trabajos. Desactivar sigue siendo la alternativa sin pérdida de trabajos. [Límites y validación de recuperación](docs/FASE_4B1_RECUPERACION.md).
...
Para trabajar en la VM `capi-dev`, seguir [fase 1: entorno completo y recuperación](docs/FASE_1_BASE_REPRODUCIBLE.md) y la [operación del compilador de fase 9](docs/FASE_9_COMPILACION_Y_DESCARGA.md#operación-dev). Usa `compose.dev.yaml`, `compose.backend.dev.yaml` y, tras instalar el compilador, **también `compose.compiler.dev.yaml`** al crear/recrear la API, más un túnel SSH privado. El recorrido vigente no exige Node en Ubuntu ni expone DEV en Internet. Desde la PC ya configurada, `.\scripts\connect-dev.ps1` permite abrir `http://localhost:3000` con la aplicación ejecutándose en la VM. La exposición HTTPS persistente está sólo [planificada en fase 13](docs/FASE_13_ACCESO_EXTERNO_DEV.md). Los proyectos confirmados se recuperan desde Mis proyectos en otros navegadores autenticados; la recuperación local y la preferencia de autoguardado pertenecen al navegador/origen. La [guía histórica de desarrollo](docs/FASE_0B_DESARROLLO.md) conserva la evidencia del editor solo.
...
Guardar el proyecto y abrir **Exportar → Compilar y descargar firmware**. Elegir Arduino o ESP-IDF y confirmar el cableado. Los programas Wi-Fi piden red y clave por separado, con consentimiento: se cifran mientras esperan y no entran en JSON/historial. **El firmware contiene la clave y no debe compartirse.**
