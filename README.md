...
El editor exige ingreso con alias y contraseña. En la VM de desarrollo corren Django + PostgreSQL, cuentas, cursos y proyectos personales. GitHub Pages ya no se utiliza. Las fases 0–9 están entregadas, la fase 10 conserva su aceptación física pendiente y las fases 11–14 están entregadas. La [fase 12](docs/FASE_12_MESA_DE_TRABAJO.md) reorganiza la mesa de trabajo, la [fase 13](docs/FASE_13_ACCESO_EXTERNO_DEV.md) publica DEV de forma persistente en [capibloques.dev.nvicchi.com](https://capibloques.dev.nvicchi.com/) y la [fase 14](docs/FASE_14_EJECUCION_VISUAL.md) incorpora paralelo vertical, ejecución sin desplazamiento automático y progreso local por camino. Las [fases 15–19](docs/PLAN_FASES_BACKLOG.md) requieren autorización propia. Producción es la **Fase final, postergada**.
...
El [backlog](docs/BACKLOG.md) distingue lo implementado en fases 12–14 y las correcciones posteriores de la barra residual, el arrastre individual/grupal y el guardado estable durante el arrastre de lo pendiente: perfiles ESP32-S3 DevKit/Waveshare de 5 pulgadas, display interactivo, TX/RX serial con bifurcaciones, desafíos progresivos, reacciones del avatar, un asistente grande para compilar/grabar y mayor transparencia y medición de la cola y compilación.
...
Para trabajar en la VM `capi-dev`, seguir la [operación pública de fase 13](docs/FASE_13_ACCESO_EXTERNO_DEV.md). `capibloques-dev-runtime` administra despliegue, validación y rollback del editor estático; no arrancar manualmente otro `editor`. La URL normal es `https://capibloques.dev.nvicchi.com/`. Desde la PC ya configurada, `.\scripts\connect-dev.ps1` conserva `http://localhost:3000` como recuperación por túnel. Los proyectos confirmados reaparecen al ingresar; la recuperación local pertenece al navegador/origen.
